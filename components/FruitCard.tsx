...
const FruitCard: React.FC<FruitCardProps> = ({ fruit, quantity, onAdd, onRemove }) => {
  ...
  useEffect(() => {
    if (quantity !== prevQuantity.current) {
      ...
      if (type === 'add') {
        // Create multiple particles flying in different directions
        const newBursts = Array.from({ length: 3 }).map((_, i) => ({
          id: Date.now() + i,
          x: (i - 1) * 30, // Left, center, right
          rotate: (i - 1) * 20
        }));
  ...
  return (
    <div className={`p-4 rounded-3xl cartoon-border transition-all duration-300 flex flex-col items-center gap-2 group relative overflow-hidden cursor-pointer
      ${isSelected ? 'bg-orange-50 border-orange-500 ring-2 ring-orange-200 shadow-lg' : 'bg-white border-[#332111]'} 
      ...
      {/* Physics-based bursts for "Add" feedback */}
      {bursts.map(burst => (
        <div 
          key={burst.id}
      ...
      <h3 className={`font-cartoon text-lg text-[#332111] transition-colors duration-300 ${isSelected ? 'text-orange-600' : 'group-hover:text-orange-500'}`}>
        {fruit.name}
      </h3>
      ...
      <p className="font-bold text-green-600 text-lg transition-transform group-hover:scale-110">₦{fruit.price.toLocaleString()}</p>
      ...
      <div className={`flex items-center gap-4 mt-4 bg-gray-100 p-2 rounded-2xl border-2 border-[#332111] transition-all duration-300 
        ...
        <div className="relative min-w-[2rem] flex justify-center">
          <span className={`font-cartoon text-2xl text-center transition-all duration-300 
            ${animationType === 'add' ? 'scale-125 text-orange-500' : animationType === 'remove' ? 'scale-75 text-red-500' : 'scale-100 text-[#332111]'}
        ...
        <button 
          ...
          className="w-10 h-10 flex items-center justify-center font-bold text-2xl text-green-500 hover:scale-125 hover:rotate-[10deg] transition-all active:scale-90"
          aria-label="Add"
        >
          +
        </button>
      ...
      {isSelected && (
        <div className="absolute top-2 right-2 bg-orange-500 text-white w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold border-2 border-white shadow-sm animate-pop">
          ✓
        </div>
      )}
    </div>
